...
def generate_labels(input_file, output_dir, clone1, clone2, alpha):
    ...
    """
    _validate_arguments(input_file, output_dir)
    df = _process_df(input_file, clone1, clone2)
    df['assignment'] = df.parallel_apply(lambda x: _get_assignment(x, alpha), axis=1)
    df.to_csv("{}/assignments.tsv".format(output_dir), sep='\t', index=False)
    _create_plot(df, clone1, clone2, output_dir)

def _shared_subclonal(mut, alpha):
    """
...
def _get_assignment(mut, alpha):
    ...
    if _shared_subclonal(mut, alpha): return 0
    elif _shared_clonal(mut, alpha): return 1
    else: return -1

def _process_df(input_file, clone1, clone2):
    """
    ...
    Returns:
        pandas.DataFrame: A DataFrame containing the processed data with columns for chromosome,
...
def _create_plot(df, clone1, clone2, output_dir):
    """
...
def _validate_arguments(input_file, output_dir):
    """
