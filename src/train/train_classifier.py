...
Functions:
    ...
    downsample_unlabeled(data, n=1000):
...
def train_classifier(filelist, model_type, no_label_prop, output_dir):
    ...
    Args:
        ...
        no_label_prop (bool): If True, indicates that the data has no labels for propagation.
...
def write_model(model, scaler, output_dir):
    """
    Save the trained model and scaler to the specified output directory.
    Parameters:
...
def compute_performance_stats(inferred, groundtruth):
    """
    Compute performance statistics for a binary classifier.
    Args:
...
def validate_arguments(filelist, output_dir):
    """
    Validates the input file and output directory.
    This function checks if the input file exists and is readable, and if the output directory exists and is writable.
    Args:
